...
# l: Expected value of the distribution
l <- 15
lim <- 3

# X intercept (Random vertical line)
x.intercept <- 18
vertical.line <- geom_vline(xintercept = x.intercept, linetype = 12, color = colors[1], size = 1)

# --- CDF ----
sup <- seq(0,lim,1)
cdf <- c(0.1, 0.4, 0.6, 1) # Comulative probabilities
...
graph2 <- ggplot(pmf.data, aes(x = sup, y = pmf)) +
  #geom_col(stat="identity", color="black", fill="transparent", width = 1, size = 1) +
  geom_point() +
  geom_segment(aes(xend = sup, yend = yend), color="grey", linetype="dashed", size=1) +
  geom_text(
  ...
  scale_x_continuous(breaks=sup, limits = c(10, 50)) +
  scale_y_continuous(labels=scaleFUN) +
  labs(x = "Value of x",
       y = "P(x) vs. f(x)") +
  theme_bw() +
...
# ---- Grid ----
grid.arrange(graph, graph2, nrow = 2, ncol = 1, layout_matrix = rbind(c(1), c(2)), top = textGrob("Discrete Distribution vs. Normal Approximation", gp = gpar(fontsize = 14)))
